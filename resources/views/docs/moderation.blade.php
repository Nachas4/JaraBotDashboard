@extends('layouts.docs-dashboard')

@section('moderation-doc')
    <div class="text-white p-2 mb-3">
        <div class="row">
            <div class="col-12 text-center text-light fs-3"><i class="fa-solid fa-user-check me-2"></i><b>Moderation Commands</b>
            </div>
        </div>
    </div>
    <div class="card--body p-3 h-100 text-white rounded overflow-auto">

        <div class="row">
            
            <div class="docs-card col-12 col-md-6 mb-5">
                <h3>Setting the Prefix</h3>

                <div class="card-- mt-3" style="height: fit-content">
                    <div>
                        <div>
                            <h4><b>getprefix</b> and <b>setprefix</b></h4>

                            <p>Usage: <span class="usage-example">&getprefix</span> and <span class="usage-example">&setprefix {prefix}</span></p>
                        </div>

                        <div>
                            <p>Get or set the bot's prefix to your heart's desire.</p>
                            <div>This can only be changed by the server owner in the <a href="{{ route('dashboard') }}">dashboard</a> (or, well, the command).</div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="docs-card col-12 col-md-6 mb-5">
                <h3>Welcome Message</h3>

                <div class="card-- mt-3" style="height: fit-content">
                    <div>
                        <div>
                            <h4><b>welcome-message</b></h4>

                            <p>Usage: <span class="usage-example">&welcome-message</span></p>
                        </div>

                        <div>
                            <p>Sends a template welcome message.</p>
                            <div>This can only be changed by the server owner in the <a href="{{ route('dashboard') }}">dashboard</a>.</div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="docs-card col-12 col-md-6 mb-5">
                <h3>Get a list of Autoroles</h3>

                <div class="card-- mt-3" style="height: fit-content">
                    <div>
                        <div>
                            <h4><b>autoroles</b></h4>

                            <p>Usage: <span class="usage-example">&autoroles</span></p>
                        </div>

                        <div>
                            <p>Get a list of all the autoroles the server has set up.</p>
                            <div>This can only be changed by the server owner in the <a href="{{ route('dashboard') }}">dashboard</a>.</div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="docs-card col-12 col-md-6 mb-5">
                <h3>Get a list of Automessages</h3>

                <div class="card-- mt-3" style="height: fit-content">
                    <div>
                        <div>
                            <h4><b>automessages</b></h4>

                            <p>Usage: <span class="usage-example">&automessages</span></p>
                        </div>

                        <div>
                            <p>Get a list of all the automessages the server has set up.</p>
                            <div>This can only be changed by the server owner in the <a href="{{ route('dashboard') }}">dashboard</a>.</div>
                        </div>
                    </div>
                </div>
            </div>

        </div>  

    </div>
@endsection
